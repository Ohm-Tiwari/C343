...
public class HashTableWithChaining<K, V> extends Dictionary<K,V>{
    ...
    public void put(K key, V value) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (entry.getKey().equals(key)) { // If the key already exists
    ...
    private void resize() {
        ...
        int nextPrimeNum = nextPrime(doubledCapacity);
        capacity = nextPrimeNum;

        List<LinkedList<Entry<K, V>>> newTable = new ArrayList<>();
        for (int i = 0; i < capacity; i++) {
        ...
        for (int i = 0; i < table.size(); i++) {
            ...
            for (int j = 0; j < list.size(); j++) {
                ...
                LinkedList<Entry<K, V>> newList = newTable.get(newIndex);
                newList.add(e);
            }
        }
        table = newTable;
    ...
    public V get(K key) {
        ...
        for (int i = 0; i < list.size(); i++) {
            Entry<K, V> entry = list.get(i);
            if (entry.getKey().equals(key)) { // If the key not found in table
